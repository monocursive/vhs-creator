...
function applyLayout(layoutKey, themeKey, currentLayers) {
	...
	const reuseTitle =
		currentLayers.find(
			(l) => l.kind === "text" && l.x >= RECTO_X && l.size > 60,
		)?.text || "TITRE\nDU FILM";
	const reuseTagline =
		currentLayers.find(
			(l) =>
				l.kind === "text" && l.x >= RECTO_X && l.size <= 18 && l.size >= 14,
		)?.text || "UNE NUIT, TOUT BASCULE";
	const reusePoster = currentLayers.find(
		(l) =>
			l.kind === "image" &&
			l.x >= RECTO_X &&
			l.src !== "assets/template/jaquette.png",
	);
	...
	return (
		...
		>
			...
			{(() => {
				...
				return bars.map((w, i) => {
					const r = (
						<rect key={i} x={x} y="4" width={w} height="36" fill="#000" />
					);
					x += w + 0.6;
					return r;
				});
...
function ThemeDecor({ theme, panel }) {
		...
		return (
			...
			>
				<defs>
					...
					<mask id="sunSlice">
						<rect width="456" height="776" fill="url(#sunMask)" />
					</mask>
				</defs>
				{/* sun glow */}
				<ellipse
	...
	if (theme.decor === "stars") {
		const stars = useMemo(
			() =>
				Array.from({ length: panel === "recto" ? 110 : 80 }).map(() => ({
					x: Math.random() * 456,
					y: Math.random() * 776,
		...
		return (
			...
			>
				{/* nebula puffs */}
				<defs>
					<radialGradient id={`nebA-${panel}`}>
						<stop offset="0" stopColor="#ff7a3a" stopOpacity="0.45" />
						<stop offset="1" stopColor="#ff7a3a" stopOpacity="0" />
					</radialGradient>
					<radialGradient id={`nebB-${panel}`}>
						<stop offset="0" stopColor="#7adfff" stopOpacity="0.35" />
						<stop offset="1" stopColor="#7adfff" stopOpacity="0" />
					</radialGradient>
					<radialGradient id={`nebC-${panel}`}>
						<stop offset="0" stopColor="#c1396a" stopOpacity="0.5" />
						<stop offset="1" stopColor="#c1396a" stopOpacity="0" />
					</radialGradient>
				</defs>
				<ellipse
					cx={panel === "recto" ? 340 : 100}
					cy="200"
					rx="200"
					ry="140"
					fill={`url(#nebA-${panel})`}
			...
			<>
				...
				<div
					style={{
						...
						height: 22,
						background:
							"repeating-linear-gradient(90deg, #d23218 0 30px, #fff4d6 30px 32px, #0e5740 32px 62px, #fff4d6 62px 64px)",
						opacity: 0.85,
						pointerEvents: "none",
					}}
				/>
	...
	if (theme.decor === "sticker" && panel === "verso") {
		return (
			...
			>
				...
				>
					N° K-7384 · DURÉE 96 MIN · COULEUR · VOSTFR
				</div>
			</div>
		);
	}
	...
	if (theme.decor === "drips" && panel === "verso") {
		return (
			...
			>
				...
				{/* edge tear */}
				<path
					d="M0 776 L 456 776 L 456 740 L 430 760 L 410 730 L 380 754 L 350 728 L 320 752 L 290 730 L 260 754 L 230 728 L 200 752 L 170 730 L 140 754 L 110 728 L 80 752 L 50 730 L 20 754 L 0 730 Z"
					fill="#0a0102"
					opacity="0.6"
				/>
			</svg>
...
function PaperFlecks({ theme }) {
	...
	return (
		...
		>
			{flecks.map((f, i) => (
				<circle key={i} cx={f.x} cy={f.y} r={f.r} fill={color} opacity={f.o} />
			))}
		</svg>
	);
}
		...
		>
			<defs>
				<pattern
					...
					x="0"
					y="0"
					width={palette.spacing}
					height={palette.spacing}
					patternUnits="userSpaceOnUse"
				>
					<circle
						cx={palette.spacing / 2}
						cy={palette.spacing / 2}
						r={palette.size / 2}
						fill={palette.dot}
					/>
				</pattern>
		...
		<div style={{ position: "absolute", inset: 0, background: t.bg }}>
			<div
				style={{
					...
					right: "15%",
					bottom: "40%",
					background: t.posterPlaceholderBg,
...
function LayoutPreview({ layoutKey }) {
	const styles = {
		...
		bigtype: (
			<>
				<div
					style={{ position: "absolute", inset: 6, background: "#3a1f5e" }}
				/>
				<div
					style={{
						position: "absolute",
			...
			<>
				<div
					style={{
						position: "absolute",
						inset: 0,
						background: "linear-gradient(135deg, #ff2d8a, #00e0ff)",
					}}
				/>
				<div
					style={{
	...
	return (
		<div style={{ position: "absolute", inset: 0 }}>{styles[layoutKey]}</div>
	);
}
...
function App() {
	...
	const [enabledLogos, setEnabledLogos] = useState({
		thx: true,
		hifi: true,
		cinemascope: true,
	});
		...
		setLayers((prev) =>
			prev.map((l) => {
				...
				return {
					...
					font: isTitle
						? T.titleFont
						: l.font.includes("SF Movie Poster") && !isTitle
							? l.font
							: isAccent
								? T.bodyFont
								: l.font,
	...
	const onFile = (e) => {
		...
		reader.onload = () => {
			...
			setLayers((ls) => {
				const placeholder = ls.find(
					(l) => l.kind === "image" && l.src === "__poster_placeholder",
				);
				if (placeholder) {
					return ls.map((l) =>
						l.id === placeholder.id ? { ...l, src: dataUrl } : l,
					);
				}
				const newL = img({
					x: RECTO_X + 60,
	...
	useEffect(() => {
		...
		setLayers((ls) => {
			...
			const enabled = Object.entries(enabledLogos)
				.filter(([, v]) => v)
				.map(([k]) => k);
			...
			enabled.forEach((key, i) => {
				...
				next = [
					...next,
					{
						...img({ x, y: baseY, w, h, src: lg.src }),
						tag: logoLayerKey(key),
						opacity: 0.95,
					},
				];
			});
	...
	const exportPNG = async () => {
		...
		if (typeof window.html2canvas === "function") {
			const c = await window.html2canvas(node, {
				scale: 2,
			...
			alert(
				"Module export en cours de chargement, réessayez dans une seconde.",
			);
		}
	};
	...
	useEffect(() => {
		if (tweaksHook) setTweaks(tweaksHook[0]);
	}, [tweaksHook?.[0]]);
	...
	return (
		<div className={`app density-${tweaks.density}`}>
			...
			<header className="topbar">
				...
				<button
					className="topbar__action topbar__action--secondary"
					onClick={undo}
					title="Annuler (⌘Z)"
				>
					↺ Annuler
				...
				<button className="topbar__action" onClick={exportPNG}>
					↓ PNG
				</button>
				<button
			...
			<aside className="rail rail--left" data-open={mobileOpen === "left"}>
				<div className="section">
					<div className="section__head">
						...
						<span className="section__title">Thème</span>
					</div>
				...
				<div className="section">
					<div className="section__head">
						<span className="section__dot" />
						<span className="section__title">Mise en page</span>
					</div>
					<div className="layouts">
						{Object.keys(LAYOUTS).map((k) => (
							<button
								key={k}
								className="layout"
								data-active={layout === k}
								onClick={() => onSelectLayout(k)}
							>
				...
				<div className="section">
					<div className="section__head">
						<span className="section__dot" />
						<span className="section__title">Affiche</span>
					</div>
					<button className="btn btn--neon" onClick={onUploadImage}>
						↥ Importer une image
					</button>
					<div style={{ height: 8 }} />
				...
				<div className="section">
					<div className="section__head">
						<span className="section__dot" />
						<span className="section__title">Logos audio / vidéo</span>
					</div>
				...
				<div className="section">
					<div className="section__head">
						<span className="section__dot" />
						<span className="section__title">Classification</span>
					</div>
				...
				<div className="section">
					<div className="section__head">
						<span className="section__dot" />
						<span className="section__title">Étiquette</span>
					</div>
					<button
						className="toggle"
						data-on={showBarcode}
			...
			>
				<div className="stage__hint">
					Cliquez sur un élément pour l'éditer · Double-clic pour modifier le
					texte
				</div>

				<div
					className="sleeve-wrap"
				...
				>
					...
					>
						...
						{/* Guides */}
						<div
							className="sleeve__guide"
							style={{ left: TRANCHE_X, width: 0 }}
						>
						...
						{tweaks.scanlineIntensity > 0 && (
							<div
								className="fx-layer fx-scanlines"
								style={{ opacity: tweaks.scanlineIntensity }}
							/>
						)}
						{tweaks.grainIntensity > 0 && (
							<div
								className="fx-layer fx-grain"
								style={{ opacity: tweaks.grainIntensity }}
							/>
						)}
				...
				{selected && !editingId && (
					<div
						style={{
							...
							left: "50%",
							top: 0,
							transform: `translate(calc(-50% + ${(selected.x + selected.w / 2 - SLEEVE_W / 2) * zoom}px), ${selected.y * zoom + 20}px)`,
						}}
					>
						<VHSLayerToolbar
							layer={selected}
				...
				<div className="stage__zoom">
					...
					<button onClick={() => setAutoFit(true)} title="Ajuster">
						⤢
					</button>
				</div>
			</main>
			...
			<aside className="rail rail--right" data-open={mobileOpen === "right"}>
				<div className="section">
					<div className="section__head">
						<span className="section__dot" />
						<span className="section__title">Inspecteur</span>
					</div>
					{!selected && (
						<div className="inspector-empty">
							Sélectionnez un élément sur la jaquette
						</div>
					)}
						...
						<>
							...
							<div className="slider-row">
								...
								<input
									...
									value={selected.size}
									onChange={(e) =>
										updateLayer(selected.id, { size: +e.target.value })
									}
								/>
							...
							<div className="slider-row">
								...
								<input
									...
									onChange={(e) =>
										updateLayer(selected.id, { letterSpacing: +e.target.value })
									}
								/>
							...
							<div className="slider-row">
								...
								<input
									...
									onChange={(e) =>
										updateLayer(selected.id, { lineHeight: +e.target.value })
									}
								/>
								<span className="val">
							...
							<div className="slider-row">
								...
								<input
									...
									onChange={(e) =>
										updateLayer(selected.id, { rotation: +e.target.value })
									}
								/>
						...
						<>
							<div className="slider-row">
								...
								<input
									...
									value={selected.w}
									onChange={(e) =>
										updateLayer(selected.id, { w: +e.target.value })
									}
								/>
							...
							<div className="slider-row">
								...
								<input
									...
									value={selected.h}
									onChange={(e) =>
										updateLayer(selected.id, { h: +e.target.value })
									}
								/>
							...
							<div className="slider-row">
								...
								<input
									...
									onChange={(e) =>
										updateLayer(selected.id, { opacity: +e.target.value })
									}
								/>
								<span className="val">
							...
							<button className="btn btn--ghost" onClick={onUploadImage}>
								↥ Remplacer l'image
							</button>
						</>
					)}
				...
				<div className="section">
					<div className="section__head">
						<span className="section__dot" />
						<span className="section__title">Aide</span>
					</div>
					<div style={{ fontSize: 11, color: "#c8b8e0aa", lineHeight: 1.5 }}>
						<p style={{ margin: "0 0 8px" }}>
							· <b>Cliquer</b> un élément pour le sélectionner
						</p>
						<p style={{ margin: "0 0 8px" }}>
							· <b>Double-cliquer</b> un texte pour le modifier
						</p>
						<p style={{ margin: "0 0 8px" }}>
							· Faire glisser pour <b>déplacer</b>, coins jaunes pour{" "}
							<b>redimensionner</b>
						</p>
						<p style={{ margin: "0 0 8px" }}>
							· Pastille bleue au-dessus pour <b>tourner</b>
						</p>
						<p style={{ margin: 0 }}>
							· <b>⌘Z / ⇧⌘Z</b> Annuler / Refaire
						</p>
					</div>
				</div>
			</aside>
			...
			<nav className="mobile-tabs">
				<button
					...
					data-active={mobileOpen === "left"}
					onClick={() => setMobileOpen(mobileOpen === "left" ? null : "left")}
				>
					Outils
				</button>
				<button
				...
				<button
					className="mobile-tab"
					data-active={mobileOpen === "right"}
					onClick={() => setMobileOpen(mobileOpen === "right" ? null : "right")}
				>
					Inspecteur
				</button>
			</nav>
