		...
		if (mode === "move") {
			...
			if (
				...
				onChange({
					...
					w,
					h,
					size: Math.max(8, Math.round(orig.size * ratio)),
				});
			} else {
				onChange({ x, y, w, h });
			}
			...
			<div
				...
				style={{
					...
					background: isPlaceholder
						? layer.placeholderBg ||
							"repeating-linear-gradient(45deg, #888, #888 10px, #999 10px, #999 20px)"
						: undefined,
				...
				{isPlaceholder && (
					<div
						style={{
							...
							background:
								layer.placeholderBg ||
								"repeating-linear-gradient(45deg, #555 0 12px, #444 12px 24px)",
...
function LayerToolbar({
	...
	return (
		...
		>
			{isText && (
				<>
					<select
						...
						title="Police"
					>
						<option value="'SF Movie Poster Cond', sans-serif">
							SF Poster Cond
						</option>
						<option value="'SF Movie Poster', sans-serif">
							SF Movie Poster
						</option>
					...
					<button
						title="Gras"
						onClick={() =>
							onChange({ weight: layer.weight === 700 ? 400 : 700 })
						}
					>
						<b>B</b>
					</button>
					...
					<button title="Centrer" onClick={() => onChange({ align: "center" })}>
						≡
					</button>
					<button
			...
			<button title="Avancer" onClick={onForward}>
				▲
			</button>
			<button title="Reculer" onClick={onBackward}>
				▼
			</button>
			<button title="Dupliquer" onClick={onDuplicate}>
				⎘
			</button>
			<span className="sep" />
			<button title="Supprimer" onClick={onDelete} style={{ color: "#ff5577" }}>
				✕
			</button>
		</div>
	);
