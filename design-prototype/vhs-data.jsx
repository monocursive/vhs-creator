...
const THEMES = {
  neon: {
    ...
    secondary: '#ff2d8a',
    titleColor: '#fffbe9',
    titleShadow: '0 0 10px #1bf0ff, 0 0 22px #1bf0ff, 5px 5px 0 #ff2d8a, 6px 6px 0 #5a0099',
  ...
  horror: {
    ...
    secondary: '#c8141a',
    titleColor: '#ff1a18',
    titleShadow: '0 0 4px #2a0a08, 4px 4px 0 #000, 6px 6px 0 #5a0a05, 0 0 24px #c8141a',
...
const RATINGS = [
  { key: 'tous',   src: 'assets/ratings/tous-publics.png',     label: 'Tous publics' },
  { key: 'parental', src: 'assets/ratings/accord-parental.png', label: 'Accord parental' },
  { key: 'i12',    src: 'assets/ratings/interdit-12.png',      label: 'Interdit -12' },
  { key: 'i16',    src: 'assets/ratings/interdit-16.png',      label: 'Interdit -16' },
  { key: 'iloc',   src: 'assets/ratings/interdit-location.png',label: 'Interdit location' },
];

/* ─────── Layer factory helpers ─────── */
let _idCounter = 1;
const newId = (prefix) => `${prefix}_${_idCounter++}_${Math.floor(Math.random() * 1e6).toString(36)}`;

function txt({ x, y, w, h, text, size = 48, font, weight = 700, color, align = 'center', shadow = '', italic = false, letterSpacing = 0, lineHeight = 1, rotation = 0 }) {
  return {
    id: newId('t'), kind: 'text',
    x, y, w, h, rotation,
    text, size, font, weight, color, align, shadow, italic,
...
function shape({ x, y, w, h, color, rotation = 0, opacity = 1, kind = 'rect' }) {
  return { id: newId('s'), kind: 'shape', shapeKind: kind, x, y, w, h, color, rotation, opacity };
}
...
function seedLayers(themeKey) {
  ...
  return [
    /* RECTO — front */
    img({ x: RECTO_X + 30, y: 60, w: RECTO_W - 60, h: 460, src: '__poster_placeholder', opacity: 1 }),
    txt({
      x: RECTO_X + 24, y: 540, w: RECTO_W - 48, h: 110,
      text: 'TITRE\nDU FILM', size: 92, font: titleFont, weight: 700,
      color: t.titleColor, shadow: t.titleShadow, align: 'center',
      letterSpacing: 2, lineHeight: 0.92,
    }),
    txt({
      x: RECTO_X + 40, y: 656, w: RECTO_W - 80, h: 30,
      text: 'UNE NUIT, TOUT BASCULE', size: 16, font: bodyFont, weight: 400,
      color: t.accent, align: 'center', letterSpacing: 4,
    }),
    txt({
      x: RECTO_X + 40, y: 690, w: RECTO_W - 80, h: 22,
      text: 'AVEC  ⋅  PRÉNOM NOM  ⋅  PRÉNOM NOM  ⋅  PRÉNOM NOM',
      size: 11, font: bodyFont, weight: 400,
      color: t.bodyColor, align: 'center', letterSpacing: 2,
    }),
    txt({
      x: RECTO_X + 40, y: 716, w: RECTO_W - 80, h: 20,
      text: 'UN FILM DE  PRÉNOM NOM',
      size: 12, font: titleFont, weight: 700,
      color: t.bodyColor, align: 'center', letterSpacing: 3,
    }),

    /* TRANCHE — spine (rotated text) */
    txt({
      x: TRANCHE_X + 10, y: 60, w: 100, h: 36,
      text: 'TITRE DU FILM', size: 26, font: titleFont, weight: 700,
      color: t.spineColor, align: 'center', letterSpacing: 2,
      rotation: 90,
    }),
    txt({
      x: TRANCHE_X + 10, y: SLEEVE_H - 130, w: 100, h: 30,
      text: 'NOM RÉAL.', size: 14, font: bodyFont, weight: 400,
      color: t.spineColor, align: 'center', letterSpacing: 2,
      rotation: 90,
    }),
    ...
    /* VERSO — back */
    txt({
      x: VERSO_X + 30, y: 50, w: VERSO_W - 60, h: 50,
      text: 'SYNOPSIS', size: 28, font: titleFont, weight: 700,
      color: t.accent, align: 'left', letterSpacing: 4,
    }),
    txt({
      x: VERSO_X + 30, y: 100, w: VERSO_W - 60, h: 220,
      ...
      size: 16, font: bodyFont, weight: 400,
      color: t.bodyColor, align: 'left', lineHeight: 1.35,
    }),
    img({ x: VERSO_X + 30, y: 340, w: 180, h: 110, src: 'assets/template/jaquette.png', opacity: 0.0 }), /* hidden seed */
    /* Rating slot — set later */
    txt({
      x: VERSO_X + 30, y: SLEEVE_H - 90, w: 240, h: 30,
      text: 'CONTRE-FICTIONS  ⋅  BOSKOP  ⋅  2026',
      size: 12, font: bodyFont, weight: 400,
      color: t.bodyColor, align: 'left', letterSpacing: 2,
    }),
    txt({
      x: VERSO_X + 30, y: SLEEVE_H - 64, w: 240, h: 24,
      text: '© LES INTERGALACTIQUES',
      size: 10, font: bodyFont, weight: 400,
      color: t.bodyColor, align: 'left', letterSpacing: 2,
    }),
  ];
}
